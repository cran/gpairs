"gpairs" <- function(x,
  ...
  if (!is.data.frame(x)) {
    if (is.matrix(x)) x <- as.data.frame(x, stringsAsFactors=TRUE)
  ...
  # Proposed addition 9/18/2024
  if (is.null(scatter.pars$frame.fill)) { scatter.pars$frame.fill <- NA } 
  ...
  scatterplot.panel <- function(x, y, type, scatter.pars, axis.pars, xpos, ypos, xylim) {
    ...
    if (type=="loess") {
      ...
      if (is(junk, "try-error")) warning("An error in loess occurred and was ignored; no line was plotted.")
    ...
    if (type=="symlm") {
      ...
      if (is(pcs, "try-error")) warning("An error in symlm occurred and was ignored; no line was plotted.")
      else {
